---
title: "5 min. Teams Test Trace: Meetings"
date: "`r Sys.Date()`"
output:
  html_document:
    df_print: paged
...
meetings <- read_csv(params$input_file, show_col_types = FALSE) %>%
  mutate(conn_type = as.factor(conn_type), teams_type = as.factor(teams_type))
```
...
meetings %>%
  ...
  ggplot(aes(x=factor(n))) +
    geom_bar(stat="count", aes(fill=teams_type)) +
...
participants <- meetings %>%
  ...
  summarize(
    ...
    audio = 15 %in% unique(teams_type),
    video = 16 %in% unique(teams_type),
    screen = 13 %in% unique(teams_type),
...
meetings %>%
  filter(teams_type == 15 & audio_112_pkts > 0) %>%
  mutate(
