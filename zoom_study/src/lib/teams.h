#ifndef TEAMS_ANALYSIS_TEAMS_H
#define TEAMS_ANALYSIS_TEAMS_H
...
namespace teams {
    ...
    struct headers {
        ...
        const unsigned char* teams_inner = nullptr;
        const unsigned char* teams_outer = nullptr;
    ...
    struct pkt {
        ...
        pkt();
        pkt(const struct teams::headers& hdr, timeval tv, bool is_p2p);
        ...
        flags flags                  = {};  //  1 Byte
        std::uint8_t teams_srv_type   = 0;   //  1 Byte
        std::uint8_t teams_media_type = 0;   //  1 Byte
    ...
    static_assert(sizeof(struct teams::pkt) == 56);
    ...
    */
    ...
    [[nodiscard]] struct headers parse_teams_pkt_buf(const unsigned char* buf,
