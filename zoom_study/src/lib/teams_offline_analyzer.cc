#include "teams_offline_analyzer.h"

void teams::offline_analyzer::add(const teams::pkt& pkt) {
    ...
    if (pkt.flags.rtp) {
        ...
        auto key = teams::media_stream_key::from_pkt(pkt);
        ...
        streams_it->second.analyzer.add(
            pkt.proto.rtp.seq, pkt.proto.rtp.ts, tv, pkt.udp_pl_len, {
                ...
                .pkt_type = pkt.teams_media_type,
                .pkts_hint = pkt.pkts_in_frame
        });
    }
}

teams::offline_analyzer::media_streams_map::iterator teams::offline_analyzer::_insert_new_stream(
        const teams::media_stream_key& stream_key, const teams::pkt& pkt) {

    // auto stream_meta = teams::media_stream_meta::from_pkt(pkt);
    ...
    auto sampling_rate = pkt.teams_media_type == 15 ? 8000 : 90000;
...
void teams::offline_analyzer::_frame_handler(const stream_analyzer& a,
...
void teams::offline_analyzer::_stats_handler(const stream_analyzer& a, unsigned report_count,
...
void teams::offline_analyzer::write_streams_log() {
    ...
    for (const auto& [key, data]: _media_streams) {
        ...
        _streams_log.stream
            << key.rtp_ssrc << ","
            << teams::media_type_to_char(key.media_type) << ","
            << teams::stream_type_to_char(key.stream_type) << ","
...
void teams::offline_analyzer::_write_pkt_log(const teams::pkt& pkt) {
...
void teams::offline_analyzer::_write_frame_log(const stream_analyzer& a, const stream_analyzer::frame& f) {
...
void teams::offline_analyzer::_write_stats_log(const teams::media_stream_key& k, unsigned report_count,
                                              unsigned ts,
    ...
    _stats_log.stream
        ...
        << k.rtp_ssrc << ","
        << teams::media_type_to_char(k.media_type) << ","
        << teams::stream_type_to_char(k.stream_type) << ","
