...
char teams::media_type_to_char(teams::media_type t) {
...
char teams::stream_type_to_char(teams::stream_type t) {
...
teams::media_stream_key teams::media_stream_key::from_pkt(const pkt& pkt) {

    enum media_type media_type;

    if (pkt.teams_media_type == teams::AUDIO_TYPE) {
        media_type = media_type::audio;
    } else if (pkt.teams_media_type == teams::VIDEO_TYPE) {
        media_type = media_type::video;
    } else if (pkt.teams_media_type == teams::P2P_SCREEN_SHARE_TYPE
        || pkt.teams_media_type == teams::SRV_SCREEN_SHARE_TYPE) {
...
bool teams::media_stream_key::operator<(const struct media_stream_key& a) const {
...
/*
teams::rtp_stream_key teams::rtp_stream_key::from_pkt(const teams::pkt& pkt) {
...
bool teams::rtp_stream_key::operator<(const struct rtp_stream_key& a) const {
...
teams::pkt::pkt(const struct teams::headers& hdr, timeval tv, bool is_p2p) {
    ...
    if (!is_p2p) {
        teams_srv_type = hdr.teams_outer[0];
        flags.to_srv = (hdr.teams_outer[7] == 0x00);
        flags.from_srv = (hdr.teams_outer[7] == 0x04);
    }

    teams_media_type = hdr.teams_inner[0];

    if (teams_media_type == 0x10) {
        pkts_in_frame = hdr.teams_inner[23];
    }
...
/*
teams::media_stream_meta teams::media_stream_meta::from_pkt(const teams::pkt& pkt) {
...
*/
struct teams::headers teams::parse_teams_pkt_buf(const unsigned char* buf, bool includes_eth, bool is_p2p) {
    ...
    if (hdr.ip->next_proto_id == 17) {
        ...
        if (is_p2p) {
            hdr.teams_inner = udp_pl;
        } else {
            hdr.teams_outer = udp_pl;

            if (hdr.teams_outer[0] == SRV_MEDIA_TYPE) {
                hdr.teams_inner = udp_pl + 8;
            }
        }
        ...
        if (hdr.teams_inner && (hdr.teams_inner[0] == AUDIO_TYPE
            || hdr.teams_inner[0] == VIDEO_TYPE
            || (is_p2p && hdr.teams_inner[0] == P2P_SCREEN_SHARE_TYPE))
            || (!is_p2p && hdr.teams_inner[0] == SRV_SCREEN_SHARE_TYPE
                && hdr.teams_inner[7] == P2P_SCREEN_SHARE_TYPE)
            || (hdr.teams_inner[0] == RTCP_SR_TYPE
                || hdr.teams_inner[0] == RTCP_SR_SD_TYPE)) {

            if (hdr.teams_inner[0] == AUDIO_TYPE) {
                hdr.rtp_rtcp_offset = hdr.udp_pl_offset + (is_p2p ? 0 : 8) + 19;
                hdr.rtp = (rtp::hdr*) (buf + hdr.rtp_rtcp_offset);
            } else if (hdr.teams_inner[0] == VIDEO_TYPE) {

                if (hdr.teams_inner[20] == 0x02) {
            ...
            } else if (is_p2p && hdr.teams_inner[0] == P2P_SCREEN_SHARE_TYPE) {
                hdr.rtp_rtcp_offset = hdr.udp_pl_offset + 20;
                hdr.rtp = (rtp::hdr*) (buf + hdr.rtp_rtcp_offset);
            } else if (!is_p2p && hdr.teams_inner[0] == SRV_SCREEN_SHARE_TYPE
                && hdr.teams_inner[7] == P2P_SCREEN_SHARE_TYPE) {
                hdr.rtp_rtcp_offset = hdr.udp_pl_offset + 35;
                hdr.rtp = (rtp::hdr*) (buf + hdr.rtp_rtcp_offset);
            } else if (hdr.teams_inner[0] == RTCP_SR_TYPE || hdr.teams_inner[0] == RTCP_SR_SD_TYPE) {
