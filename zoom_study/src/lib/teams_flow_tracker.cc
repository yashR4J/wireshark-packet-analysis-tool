...
#include "pcap_util.h"
#include "teams_flow_tracker.h"
#include "teams_nets.h"

bool teams::flow_tracker::flow_stats::is_udp() const {
    return type == flow_type::udp_srv || type == flow_type::udp_p2p || type == flow_type::udp_stun;
}

bool teams::flow_tracker::flow_stats::is_stun() const {
    return type == flow_type::udp_stun;
}

bool teams::flow_tracker::flow_stats::is_tcp() const {
    return type == flow_type::tcp;
}

bool teams::flow_tracker::flow_stats::is_p2p() const {
    return type == flow_type::udp_p2p;
}

teams::flow_tracker::flow_tracker(unsigned int stun_expiration)
    : _stun_expiration(stun_expiration) { }

std::optional<teams::flow_tracker::flow_stats> teams::flow_tracker::track(
    ...
    if (flows_it != _flows.end()) { // flow has been seen before
        ...
        _teams_pkts_detected++;
        _teams_bytes_detected += bytes;
        ...
        if (teams::nets::match(ip_5t.ip_src) || teams::nets::match(ip_5t.ip_dst)) {
            // flow is going to / coming from teams server
        ...
        } else { // flow is not going to / coming from teams server
...
unsigned teams::flow_tracker::count_teams_flows_detected() const {
    return _next_id;
}

unsigned long long teams::flow_tracker::count_total_pkts_processed() const {
    return _total_pkts_processed;
}

unsigned long long teams::flow_tracker::count_teams_pkts_detected() const {
    return _teams_pkts_detected;
}

unsigned long long teams::flow_tracker::count_teams_bytes_detected() const {
    return _teams_bytes_detected;
}

const std::unordered_map<net::ipv4_5tuple, teams::flow_tracker::flow_stats>&
    teams::flow_tracker::flows() const {
