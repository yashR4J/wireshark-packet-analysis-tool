...
void teams::analyzer::enable_pkt_log(const std::string& file_path) {
...
void teams::analyzer::enable_frame_log(const std::string& file_path) {
...
void teams::analyzer::enable_streams_log(const std::string& file_path) {

    _streams_log.open(file_path);
}

void teams::analyzer::enable_stats_log(const std::string& file_path) {
...
void teams::analyzer::_log::open(const std::string& file_path) {

    stream.open(file_path);
    enabled = true;

    if (!stream.is_open())
        throw std::runtime_error("teams::analyzer: could not open log file at " + file_path);
}

void teams::analyzer::_log::close() {

    if (!stream.is_open())
        throw std::logic_error("teams::analyzer: could not close log file: file is not open");
