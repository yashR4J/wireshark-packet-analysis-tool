...
namespace teams_meetings {
    ...
    cxxopts::Options set_options() {
        ...
        cxxopts::Options opts("teams_meetings", "");
...
//          ("t,timeout", "timeout in sec. (default: 3600)", cxxopts::value<unsigned>(), "T")
    ...
    struct stream_key {
        ...
        std::uint8_t teams_type = 0;
        bool p2p = false;

        bool operator<(const stream_key &other) const {

            return std::tie(ssrc, ip_src, tp_src, ip_dst, tp_dst, teams_type, p2p) <
                   std::tie(other.ssrc, other.ip_src, other.tp_src, other.ip_dst, other.tp_dst,
                            other.teams_type, other.p2p);
        }

        static struct stream_key from_pkt(const teams::pkt& pkt) {
            ...
            if (pkt.flags.rtp) {
                return stream_key {
                    ...
                    .teams_type = pkt.teams_media_type,
    ...
    struct stream_state {
        ...
        void update_with_pkt(const teams::pkt& pkt) {
