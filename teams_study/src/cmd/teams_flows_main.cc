...
int main(int argc, char** argv) {
    ...
    auto config = teams_flows::parse_options(teams_flows::set_options(), argc, argv);
    pcap_file_writer pcap_out;
    std::ofstream flows_out, types_out, rate_out;
    simple_binary_writer<teams::pkt> zpkt_writer;
    ...
    pcap_pkt pkt;
    teams::flow_tracker flow_tracker;
    ...
    unsigned last_ts = 0;
    std::uint64_t last_total_pkt_count = 0, last_teams_pkt_count = 0, last_teams_byte_count = 0;
    ...
    while (pcap_in.next(pkt)) {
        ...
        if (config.rate_out_file_name) {
            ...
            if (last_ts == 0) {
                ...
                rate_out << "ts_s,total_pkts,teams_pkts,teams_bytes" << std::endl;
            }

            if (pkt.ts.tv_sec > last_ts) {

                std::uint64_t current_total_pkt_count = mac_counter.count();
                std::uint64_t current_teams_pkt_count = flow_tracker.count_teams_pkts_detected();
                std::uint64_t current_teams_byte_count = flow_tracker.count_teams_bytes_detected();

                rate_out << last_ts << ","
                         << (current_total_pkt_count - last_total_pkt_count)
                         << "," << (current_teams_pkt_count - last_teams_pkt_count)
                         << "," << (current_teams_byte_count - last_teams_byte_count)
                         << std::endl;

                last_ts = pkt.ts.tv_sec;
                last_total_pkt_count = current_total_pkt_count;
                last_teams_pkt_count = current_teams_pkt_count;
                last_teams_byte_count = current_teams_byte_count;
            }
        }
        ...
        auto teams_flow = flow_tracker.track(ip_5t, pkt.ts, pkt.frame_len);

        if (teams_flow) {

            // p2p-only option:
            if (config.p2p_only && !teams_flow->is_p2p() && !teams_flow->is_stun()) continue;

            auto hdr = teams::parse_teams_pkt_buf(pkt.buf, true, teams_flow->is_p2p());

            if (teams_flow->type == teams::flow_tracker::flow_type::udp_p2p) {
                p2p_inner_types[hdr.teams_inner[0]].increment(1, ntohs(hdr.udp->dgram_len));
            } else if (teams_flow->type == teams::flow_tracker::flow_type::udp_srv) {

                srv_outer_types[hdr.teams_outer[0]].increment(1, ntohs(hdr.udp->dgram_len));

                if (hdr.teams_outer[0] == teams::SRV_MEDIA_TYPE) {
                    srv_inner_types[hdr.teams_inner[0]].increment(1, ntohs(hdr.udp->dgram_len));
                }
            }

            if (config.zpkt_out_file_name && teams_flow->is_udp()) {
                teams::pkt zpkt(hdr, pkt.ts, teams_flow->is_p2p());
    ...
    if (config.flows_out_file_name) {
        ...
        for (const auto& [ip_5t, stats]: flow_tracker.flows()) {
            flows_out << stats.id << "," << ip_5t << ","
                      << teams::flow_tracker::flow_type_string(stats.type) << "," << stats.pkts << ","
    ...
    std::cout << "- teams pkts: " << flow_tracker.count_teams_pkts_detected() << std::endl;
    std::cout << "- teams flows: " << flow_tracker.count_teams_flows_detected() << std::endl;
