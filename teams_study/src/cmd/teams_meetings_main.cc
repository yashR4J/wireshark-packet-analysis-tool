...
#include "teams_meetings.h"

using namespace teams_meetings;
...
struct streams {
    ...
    std::optional<unsigned> find_duplicate(const teams::pkt& pkt, unsigned buffer_s = 3000) {
    ...
    void print_csv_to_stream(std::ostream& os) const {
        ...
        os  << "stream_id,conn_type,start_ts_s,end_ts_s,ip_src,tp_src,ip_dst,tp_dst,teams_type,ssrc,"
        ...
        for (const auto& [ssrc, stream_map] : data) {
            for (const auto& [stream_key, stream_state] : stream_map) {
                ...
                os  << (stream_state.stream_id ? std::to_string(*stream_state.stream_id) : "NA") << ","
                    ...
                    << stream_key.tp_dst << ","
                    << (unsigned) stream_key.teams_type << ","
                    << ssrc << ","
...
public:
    ...
    void print_meetings_csv_to_stream(std::ostream& os) {
        ...
        os  << "meeting_id,stream_id,conn_type,start_ts_s,end_ts_s,ip_src,tp_src,ip_dst,tp_dst,"
            << "teams_type,ssrc,start_rtp_ts,end_rtp_ts,pkts,bytes,audio_112_pkts,audio_99_pkts,"
            << "audio_113_pkts"
            << std::endl;
...
private:
    ...
    static net::ipv4_port _client_ip_port(const stream_key& k) {
        ...
        if (teams::nets::match(k.ip_src)) { // src is a teams server -> return dst
            return net::ipv4_port{k.ip_dst, k.tp_dst};
        } else if (teams::nets::match(k.ip_dst)) { // dst is a teams server -> return src
            return net::ipv4_port{k.ip_src, k.tp_src};
        } else { // neither is a teams server -> return smaller ip address
...
int main(int argc, char** argv) {
    ...
    simple_binary_reader<teams::pkt> zpkt_reader(config.input_file_name);

    teams::pkt pkt;
    struct { unsigned long total_pkts = 0, media_pkts = 0, streams = 0; } counters;

    auto is_media_pkt = [](const teams::pkt& pkt) {
